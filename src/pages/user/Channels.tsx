...
const UserChannels = () => {
  ...
  const { channels, categories, plans } = useData();
  ...
  const filteredChannels = useMemo(() => {
    return channels.filter(c => {
      ...
      const isAllowed = userPlan?.categoryIds.includes(c.categoryId);
      return matchesSearch && matchesCategory && isAllowed;
    });
  }, [channels, search, selectedCategory, userPlan]);
    ...
    <UserLayout>
      <header className="mb-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <h1 className="text-4xl font-black tracking-tighter text-white">Todos os Canais</h1>
            <p className="text-slate-500 text-sm font-medium">Explore nossa grade completa de programação.</p>
          </div>
          ...
          <div className="relative w-full md:w-96">
            ...
            <input 
              ...
              className="w-full bg-slate-900 border border-white/5 rounded-2xl py-3.5 pl-12 pr-4 text-sm focus:outline-none focus:border-red-500/50 transition-all"
            />
          </div>
        </div>
      ...
      {filteredChannels.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {filteredChannels.map((channel) => (
            <ChannelCard 
              key={channel.id} 
              channel={channel} 
              isFavorite={user?.favorites?.includes(channel.id)}
        ...
        <div className="flex flex-col items-center justify-center py-20 text-slate-600">
          <Tv size={60} className="mb-4 opacity-20" />
          <h3 className="text-xl font-black text-white mb-2">Nenhum canal encontrado</h3>
          <p className="text-sm font-medium">Tente ajustar seus filtros ou busca.</p>
        </div>
      )}
...
// Reusable Components (Same as Home.tsx but could be moved to separate files if project grows)
      ...
      <div className="p-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest truncate">Ao Vivo</p>
          </div>
        </div>
      </div>
    </motion.div>
